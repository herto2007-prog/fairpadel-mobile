...
import {
  ChevronLeft, ChevronRight, MapPin, CalendarDays, Wallet, Users, Trophy, Clock, Award, AlertCircle, GitBranch,
...
function ActionCard({ icon, title, sub, onPress, accent }: { icon: React.ReactNode; title: string; sub: string; onPress: () => void; accent?: string }) {
  return (
    <TouchableOpacity style={[styles.action, accent ? { borderColor: accent } : null]} activeOpacity={0.85} onPress={onPress}>
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.org}>Organiza {torneo.organizador.nombre} {torneo.organizador.apellido}</Text>
          ...
          <View style={styles.actionsGrid}>
            <ActionCard
              ...
              title="Inscriptos"
              sub={`${torneo.totalInscritos} ${torneo.totalInscritos === 1 ? 'pareja' : 'parejas'}`}
              onPress={goInscriptos}
            />
            <ActionCard
              icon={<GitBranch size={22} color={colors.primary} />}
              title="Llave / Cuadro"
              sub="Por categoría"
              onPress={goLlave}
            ...
            {esFinalizado && (
              <ActionCard
                icon={<Trophy size={22} color={colors.amber500} />}
                title="Campeones"
                sub="Ver el podio"
                onPress={goCampeones}
              />
...
const styles = StyleSheet.create({
  ...
  infoCard: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: radius.lg, padding: spacing.md, marginTop: spacing.md, gap: spacing.md },
  infoRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md },
  infoIcon: { width: 38, height: 38, borderRadius: radius.md, backgroundColor: colors.dark100, alignItems: 'center', justifyContent: 'center' },
  infoLabel: { color: colors.gray400, fontSize: 12 },
  infoValue: { color: colors.white, fontSize: 15, fontWeight: '600', marginTop: 1 },
  // Botones de acción
  actionsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md - 4, marginTop: spacing.md },
  action: { width: '47.8%', backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: radius.lg, padding: spacing.md },
  actionTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  actionTitle: { color: colors.white, fontSize: 15, fontWeight: '700', marginTop: spacing.sm },
  actionSub: { color: colors.gray400, fontSize: 12, marginTop: 2 },
  section: { marginTop: spacing.lg },
  sectionTitle: { color: colors.white, fontSize: 17, fontWeight: '800', marginBottom: spacing.sm },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  chip: { backgroundColor: colors.dark100, borderWidth: 1, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 7 },
  ...
  bottomBar: { position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: colors.card, borderTopWidth: 1, borderTopColor: colors.border, paddingHorizontal: spacing.lg, paddingTop: spacing.md },
  cta: { backgroundColor: colors.primary, borderRadius: radius.lg, height: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm },
